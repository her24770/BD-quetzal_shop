...
def buscar_por_nit(nit: str, current_user: TokenData = Depends(get_current_user)):
    return cliente_controller.get_by_nit(nit)


@router.get("/{cliente_id}", response_model=ClienteResponse)
def obtener(cliente_id: int, current_user: TokenData = Depends(get_current_user)):
    return cliente_controller.get_by_id(cliente_id)


@router.post("/", response_model=ClienteResponse, status_code=201)
def crear(body: ClienteCreate, current_user: TokenData = Depends(get_current_user)):
    return cliente_controller.create(body)


@router.patch("/{cliente_id}", response_model=ClienteResponse)
def actualizar(cliente_id: int, body: ClienteUpdate, current_user: TokenData = Depends(get_current_user)):
    return cliente_controller.update(cliente_id, body)


@router.delete("/{cliente_id}")
def eliminar(cliente_id: int, current_user: TokenData = Depends(get_current_user)):
