...
def listar(current_user: TokenData = Depends(require_permission("compras", "SELECT"))):
    return compra_controller.get_all()


@router.get("/{compra_id}", response_model=CompraResponse)
def obtener(compra_id: int, current_user: TokenData = Depends(require_permission("compras", "SELECT"))):
    return compra_controller.get_by_id(compra_id)


@router.post("/", response_model=CompraResponse, status_code=201)
def crear(body: CompraCreate, current_user: TokenData = Depends(require_permission("compras", "INSERT"))):
